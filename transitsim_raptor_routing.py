        ...
        for trip in all_trips:
            ...
            if trips.shape[0] == 0:
                ...
                if not os.path.exists(
                    ...
                    if trips.shape[0] > 0:
                        print(f'{mode} routing from {taz_name} at {start_time}:')
...
def one_trip(trips,impedance,raptor_settings,start_time):
...
# start = '207'
# end = '642'
# start_time = datetime(2022, 11, 24, 9, 0, 0, 0)

# #import trips
# all_trips = pd.read_parquet(trips_dir + rf'/{start}.parquet')

# #filter
# trips = all_trips.loc[all_trips['dest_taz']==end,:]

# #run
# output = one_trip(trips,impedance,raptor_settings,start_time)
...
raptor_settings = {
    ...
    'first_time': datetime(2022, 11, 24, 9, 0, 0, 0), #9am
    'end_time': datetime(2022, 11, 24, 10, 0, 0, 0), #10am
    'timestep': timedelta(minutes=20), #in minutes 
